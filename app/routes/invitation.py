...
async def create_join_request(request_data: JoinRequestCreate,
                             service: InvitationService = Depends(get_invitation_service),
                             current_user: User = Depends(get_current_user)):
    return await service.create_join_request(request_data, current_user)
...
async def get_my_sent_requests(current_user: User = Depends(get_current_user),
                               service: InvitationService = Depends(get_invitation_service)):
    return await service.get_user_sent_requests(current_user)

@router.get("/my/invitations", response_model=List[MembershipResponse])
async def get_my_received_invitations(current_user: User = Depends(get_current_user),
                                      service: InvitationService = Depends(get_invitation_service)):
    return await service.get_user_received_invitations(current_user)
...
async def get_company_invited_users(company_id: int,
                                    current_user: User = Depends(get_current_user),
                                    service: InvitationService = Depends(get_invitation_service)):
    return await service.get_owner_sent_invitations(company_id, current_user)
...
async def get_company_pending_join_requests(company_id: int,
                                            current_user: User = Depends(get_current_user),
                                            service: InvitationService = Depends(get_invitation_service)):
    return await service.get_company_pending_requests(company_id, current_user)
